function h = viewAnnotations(dataset, layerName,startAnnot,channels,savePrefix)
%DOES NOT WORK ON MATLAB v2014b or later due to graphical handling changes
...
%     end
    ...
    %add Validation bar
    uicontrol(uihandle,'Style', 'pushbutton','Units','centimeters', 'String', 'Correct',...
    'Position', [0.1 0.1 4 1], 'BackgroundColor', [0 1 0], 'Callback',@CorrectLbl,'Parent',bottomPanel);

    uicontrol(uihandle,'Style', 'pushbutton','Units','centimeters', 'String', 'Uncertain',...
    'Position', [4.9 0.1 4 1], 'BackgroundColor', [1 1 0], 'Callback',@UncertainLbl,'Parent',bottomPanel);
    ...
    anns=getAllAnnots(dataset,layerName);
